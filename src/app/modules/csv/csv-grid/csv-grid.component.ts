...
export class CsvGridComponent implements OnInit, OnDestroy {
  ...
  constructor(private _zone: NgZone,
        ...
        payload => {
          ...
          if (payload.action === "CSV LAYERSYNC LAYERINFO") {
            ...
            this.parentMapId = payload.value;
              ...
              this.plotMarker(tracks, payload.value.showLabels,
                payload.value.color, payload.value.showZoom, (payload.action === "CSV SAVE TO CATALOG"),
                false, payload.value.mapId);
            }
        });
    }
    ...
    plotMarker(selectedRows, labels, color, zoom, share, temporary, mapId) {
      ...
      this.worker.postMessage({
        ...
        tracks: selectedRows,
        showLabels: labels, color: color, columnTracking: this.columnTracking,
        mmsiEnabled: (this.layerMMSIFieldName !== ""),
        baseUrl: this.configService.getBaseHref(),
        saveToCatalog: share, showZoom: zoom, mapId: mapId
      });
